import type { HarnessStage, HarnessMode, TaskProfile, Difficulty } from './types'
...
export function canTransition(opts: CanTransitionOpts): { ok: true } | { ok: false; reason: string } {
  ...
  if (effectiveStageRequirement(opts.profile, opts.difficulty, opts.to) === 'forbidden') {
  ...
  // Allow extra edges in fast mode (brainstorm → search, spec → implement, plan → implement)
  if (opts.mode === 'fast') {
    const fastEdges: Array<[HarnessStage, HarnessStage]> = [
      ['brainstorm', 'search'],
      ['spec', 'implement'],
      ['plan', 'implement'],
    ]

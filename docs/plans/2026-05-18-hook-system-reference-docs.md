# Hook System Reference Docs Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Land a single authoritative reference document (`docs/hooks-reference.md`) that covers every in-process hook event, every built-in handler, every extension point (plugin `inProcessHooks:` sidecar + `~/.nuka/hooks.config.{js,mjs}`), the pipeline-vs-last-write-wins semantics for `afterToolCall` and the last-write-wins rules for `afterAssistantMessage`, and the environment variables that gate each opt-in handler.

**Architecture:** The new doc lives alongside `docs/plugin-hooks.md` (which stays as the plugin-author-focused guide). The reference doc is the operator / integrator surface — exhaustive matrices, code examples, and environment-variable inventory. It cross-links to `plugin-hooks.md` for the plugin-only manifest details. The doc is regenerable: every fact it states is sourced from a file under `src/core/hooks/` or from the registration block in `src/cli.tsx`, and the validation tasks below grep those sources to catch drift.
...
```
docs/hooks-reference.md             # CREATE — new reference doc
docs/plugin-hooks.md                # READ — link from new doc, do not modify
src/core/hooks/events.ts            # READ — event enum + payload shapes
src/core/hooks/registry.ts          # READ — register / invoke surface
src/core/hooks/pipeline.ts          # READ — sequential + abort + error isolation
src/core/hooks/lifecycle.ts         # READ — fire helpers + payload types
src/core/hooks/wrapTool.ts          # READ — pipeline vs last-write-wins semantics
src/core/hooks/configLoader.ts      # READ — user config extension
src/cli.tsx                         # READ — built-in handler registration + env vars
```

The deliverable is a single markdown file. No source code changes.

---

## Task 2.0 — Verify the event inventory matches source

- [ ] Files
  - Read: `src/core/hooks/events.ts`

- [ ] Steps
  1. Confirm the `IN_PROCESS_HOOK_EVENTS` array contains exactly these 13 entries (in declaration order):
     ```bash
     grep -E "^\s*'[a-zA-Z]+'," src/core/hooks/events.ts
     ```
     Expected names:
     - `beforeToolCall`, `afterToolCall`, `afterToolCallFailure`
     - `afterTurn`, `beforeAutoCompact`, `promptSubmit`, `promptRendered`
     - `sessionStart`, `sessionEnd`, `subagentStart`, `notification`
     - `shellHookExecuted`
     - `afterAssistantMessage`
  2. If the list drifts (a future iter adds or renames an event) → update the reference doc's "Event matrix" section before merging the doc.

---

## Task 2.1 — Verify the built-in handler inventory matches source

- [ ] Files
  - Read: `src/cli.tsx`

- [ ] Steps
  1. Grep the registration block for the 7+1 known built-in handler IDs:
     ```bash
     grep -nE "id: '(recentFiles-auto-touch|auto-truncate-output|path-display-rewriter|json-format-pretty-printer|word-wrap-rewriter|url-extract-annotator|applyDiff-permission-gate|whitespace-normalize-observer)'" src/cli.tsx
     ```
     Confirm 8 hits.
  2. For each, note: event slot, factory function, env-var gate (if any). The doc's "Built-in handlers" matrix must match this enumeration exactly.

---

## Task 2.2 — Draft Section 1 (Overview + two-subsystem framing)

- [ ] Files
  - Create: `docs/hooks-reference.md` (overwrite if it exists)

- [ ] Steps
  1. Write the file header + first section. Exact content to land:
     ````markdown
     ...
     ---
     ````
  2. Save the file with that content as its head.

---

## Task 2.3 — Draft Section 2 (Event matrix)

- [ ] Files
  - Modify: `docs/hooks-reference.md`

- [ ] Steps
  1. Append the event matrix. Exact content:
     ````markdown
     ## Event matrix
...
## Task 2.4 — Draft Section 3 (Pipeline semantics + last-write-wins)

- [ ] Files
  - Modify: `docs/hooks-reference.md`

- [ ] Steps
  1. Append the semantics section. Exact content:
     ````markdown
...
## Task 2.5 — Draft Section 4 (Built-in handlers matrix + env-var matrix)

- [ ] Files
  - Modify: `docs/hooks-reference.md`

- [ ] Steps
  1. Append the matrices. Exact content:
     ````markdown
     ## Built-in handlers
...
## Task 2.6 — Draft Section 5 (Extension points + worked examples)

- [ ] Files
  - Modify: `docs/hooks-reference.md`

- [ ] Steps
  1. Append the extension-points section with three runnable examples. Exact content:
     ````markdown
     ## Extension points
...
## Task 2.7 — Validate code examples compile (TypeScript signature check)

- [ ] Files
  - Read: `docs/hooks-reference.md` (the just-written file)
  - Test: ad-hoc `npx tsc --noEmit` scratch-files for each TS-typed example

- [ ] Steps
  1. For each TypeScript fragment in the doc that declares a type (the `HookConfigEntry` snippet in §"User config"), confirm it matches `src/core/hooks/configLoader.ts:HookConfigEntry`:
     ```bash
     grep -A6 "^export interface HookConfigEntry" src/core/hooks/configLoader.ts
     ```
     The doc must list exactly `event`, `handler`, optional `id`, optional `priority`. No drift.
  2. For each `.mjs` example (Examples A / B / C):
     - Confirm `event` strings are members of `IN_PROCESS_HOOK_EVENTS`:
       ```bash
       grep -nE "^\s*'(afterToolCall|promptSubmit|beforeToolCall|afterAssistantMessage)'" src/core/hooks/events.ts
       ```
     - Confirm the handler return-shape (`{ data: { replaceResult: ... } }`) matches the `wrapTool.ts:isToolResult` gate (object with `boolean isError` and `string | array output`).
  3. Smoke-test Example C end-to-end by dropping it into a scratch `~/.nuka/hooks.config.mjs` under a tmp `$HOME`, running `npx tsx src/cli.tsx`, and confirming the boot log shows `[nuka:hooks] config errors from ... : ` is NOT emitted. Tear down the scratch home.

---

## Task 2.8 — Verify the doc against source (anti-drift sweep)

- [ ] Files
  - Read: `docs/hooks-reference.md`, `src/core/hooks/events.ts`, `src/cli.tsx`

- [ ] Steps
  1. Confirm every event listed in the doc appears in `IN_PROCESS_HOOK_EVENTS`:
     ```bash
     for evt in beforeToolCall afterToolCall afterToolCallFailure afterTurn beforeAutoCompact promptSubmit promptRendered sessionStart sessionEnd subagentStart notification shellHookExecuted afterAssistantMessage; do
       grep -q "'$evt'" src/core/hooks/events.ts && echo "OK $evt" || echo "MISSING $evt"
     done
     ```
     Expected: all 13 print `OK`.
  2. Confirm every built-in handler ID in the doc is registered in `src/cli.tsx`:
     ```bash
     for id in recentFiles-auto-touch auto-truncate-output path-display-rewriter json-format-pretty-printer word-wrap-rewriter url-extract-annotator applyDiff-permission-gate whitespace-normalize-observer; do
       grep -q "id: '$id'" src/cli.tsx && echo "OK $id" || echo "MISSING $id"
     done
     ```
     Expected: all 8 print `OK`.
  3. Confirm every env-var listed in the matrix is consumed in `src/cli.tsx`:
     ```bash
     for v in NUKA_HOOK_PIPELINE_MODE NUKA_RECENT_FILES_NO_PERSIST NUKA_PATH_DISPLAY_HOOK NUKA_JSON_FORMAT_HOOK NUKA_WORD_WRAP_HOOK NUKA_WORD_WRAP_WIDTH NUKA_URL_EXTRACT_HOOK NUKA_APPLY_DIFF_ALLOWED_ROOTS NUKA_WHITESPACE_HOOK; do
       grep -q "$v" src/cli.tsx && echo "OK $v" || echo "MISSING $v"
     done
     ```
     Expected: all 9 print `OK`.
  4. If any line prints `MISSING`, the doc is out-of-date — update the doc (not the source) before merging.

---

## Task 2.9 — Render check

- [ ] Files
  - Read: `docs/hooks-reference.md`

- [ ] Steps
  1. Confirm the doc renders without markdown errors:
     ```bash
     npx --yes markdownlint-cli2 docs/hooks-reference.md 2>&1 || true
     ```
     (Use `|| true` because we have no `markdownlint-cli2` dependency; the run is advisory. The doc must at minimum open in a viewer without raw HTML/escape glitches.)
  2. Confirm all internal anchor links resolve:
     ```bash
     grep -oE "#[a-z0-9-]+" docs/hooks-reference.md | sort -u
     ```
     Verify each anchor corresponds to a `## ` or `### ` heading slug in the file.
  3. Confirm cross-links to sibling docs resolve:
     ```bash
     test -f docs/plugin-hooks.md && test -f docs/plugins.md && echo OK
     ```
     Expected: `OK`.
...
| Section | Source of truth | Drift check |
|---|---|---|
| Event matrix | `src/core/hooks/events.ts:IN_PROCESS_HOOK_EVENTS` | Task 2.8 step 1 |
| Built-in handlers | `src/cli.tsx` registration block | Task 2.8 step 2 |
| Env-var matrix | `src/cli.tsx` `process.env.*` reads | Task 2.8 step 3 |
| Pipeline semantics | `src/core/hooks/wrapTool.ts` header + body | Task 2.7 — manual cross-read |
| afterAssistantMessage rules | `src/core/hooks/events.ts` (afterAssistantMessage block) + `lifecycle.ts:extractReplaceText` / `applyReplaceTextToAssistant` | Task 2.7 — manual cross-read |
| User config paths | `src/core/hooks/configLoader.ts:defaultHookConfigPaths` | Task 2.7 step 1 |
| Plugin manifest field | `docs/plugin-hooks.md` | Cross-link only, no duplication |

Honesty constraint: no claim in the doc should be left unsourced. Every event, every handler ID, every env-var maps back to a line in `src/core/hooks/` or `src/cli.tsx`. The Task 2.8 grep loops are the regression net against drift when a future iter adds an event or renames a handler.

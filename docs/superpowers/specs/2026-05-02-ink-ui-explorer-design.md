...
│   │   ├── …
│   │   └── regression/              ← auto-generated by L4
│   │       └── 2026-05-02-welcome-flexgrow.tsx
│   └── .ink-explorer/               ← gitignored
│       ├── failures/<id>.md
│       ├── judge-cache.json
│       └── runs/<timestamp>.jsonl   ← raw run log
└── .gitignore                        ← + .ink-explorer/
```

No vitest, ink, or tsconfig changes required. The runner discovers fixtures by
glob, registers them, and runs out-of-process from the project's own test
suite.
...
3. **Verify** (`verify.ts`): on subagent's `verify` tool calls — fork the
   runner in-process, mount the fixture, return JSON.
